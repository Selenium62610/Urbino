...
class Engine extends OpenXum.Engine {
  ...
  tests(){
    ...
    this.move(new Move(8,4,new Building(Color.WHITE,BuildingType.TOWER,new Coordinates(8,4))));

    this.move(new Move(2,8,new Building(Color.WHITE,BuildingType.TOWER,new Coordinates(2,8))));
  ...
  move(move) {
    ...
    if(this._phase === Phase.BUILDING) {
      _manager.buildingList().push(move._piece);
      this.nbPass = 0;
      ...
      let left = this._board[this.getIndex(x, y - 1)];
      //down
    console.log("A BUILDING IS MOVING");
      if (top instanceof Building) {


        if (top.idDistrict() >= tmpDistrict) {
          tmpDistrict = top.idDistrict();

        }


        if (top.idBlock() >= tmpBlock && top.color() ===this._board[this.getIndex(x , y)].color() ) {
          console.log("top.idBlock() >= tmpBlock && top.color() === this.current_color()");
          tmpBlock = top.idBlock()+1;
          for (const item of _manager.buildingList()) {
            if (item.idBlock() === top.idBlock()) {

              console.log("propa");
              item._idBlock = tmpBlock+1;
            }
      ...
      if (down instanceof Building) {
        ...
        if (down.idDistrict() >= tmpDistrict ) {
          tmpDistrict = down.idDistrict();
        }

        if (down.idBlock() <= tmpBlock && down.color() === this._board[this.getIndex(x , y)].color()) {
      ...
      if (left instanceof Building) {
        ...
        if (left.idDistrict() >= tmpDistrict ) {
          tmpDistrict = left.idDistrict();
        }
        if (left._idBlock() <= tmpBlock && left.color() === this._board[this.getIndex(x , y)].color()) {
      ...
      if (right instanceof Building) {
        ...
        if (right.idDistrict() >= tmpDistrict) {
          tmpDistrict = right.idDistrict();
        }
        if (right.idBlock() <= tmpBlock && right.color() === this._board[this.getIndex(x , y)].color()) {
      ...
      if (move._piece._idBlock === this.nbBlock)
        this.nbBlock++;
      ...
      if (tmpDistrict === this.nbDistrict) {
        ...
        if (tmpDistrict == -1) {
          ...
          move._piece._idDistrict = this.nbDistrict;
          this.nbDistrict++;
        }
  ...
  countPoints(){
    ...
    for(const item of _manager.buildingList()){
      ...
      if(item.idDistrict() > 1){
        list[item.idDistrict()].push(item);
        console.log("added");
      }
