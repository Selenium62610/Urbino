...
class Engine extends OpenXum.Engine {
  ...
  move(move) {
    ...
    console.log(move);
      ...
      if(move._coord.x()===-20 && move._coord.y()===-20 ) {
          console.log("coord de pas bo qui bouge pas l'architect");
          this._phase = Phase.BUILDING;

      }else if (move._coord.x()===-30 && move._coord.y()===-30){
          console.log("plus de mouvement possible de building");
          this._phase=Phase.ARCHITECT;
          this.next_color();

      }else if(this._phase === Phase.ARCHITECT){
          console.log("IN PHASE ARCHITECT");

          console.log(move);
          this._board[this.getIndex(move._coord.x(),move._coord.y() )] = 0;
