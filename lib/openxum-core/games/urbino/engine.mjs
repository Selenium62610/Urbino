...
class Engine extends OpenXum.Engine {
   ...
   tests(){
        ...
        this._board[this.getIndex(2, 1)] = new Building(Color.WHITE, BuildingType.HOUSE, new Coordinates(2, 1));
      ...
      switch (this._phase) {
        ...
        case Phase.BUILDING:
          ...
          for (const _item of this.getIntersections()) {
            ...
            //moves.concat(new Move(i,j,Building));
    ...
    move(move) {
        ...
        if(this._phase === Phase.BUILDING) {
            ...
            if(top instanceof Building ){
                ...
                tmpBlock= top.idBlock();
                    for (const item of Building){
                        if (item._idBlock()==top.idBlock()){
                            item.idBlock=tmpBlock;
                        }
            ...
            if(down instanceof Building ){
                ...
                if(down.idBlock()<tmpBlock && down.color()===this._color) {
                    tmpBlock = down.idBlock();
                    for (const item of Building){
                        if (item._idBlock()==down.idBlock()){
                            item.idBlock=tmpBlock;
                        }
            ...
            if(left instanceof Building ){
                ...
                if(left.idBlock()<tmpBlock && left.color()===this._color) {
                    tmpBlock = left.idBlock();

                    for (const item of Building){
                        if (item._idBlock()==left.idBlock()){
                            item.idBlock=tmpBlock;
                        }
