...
class Gui extends OpenXum.Gui {
  ...
  get_move() {
    ...
    if(this._engine._phase === Phase.ARCHITECT) {
      ...
      if(this._engine._phase === Phase.BUILDING) {
          console.log("COLOR IN GET MOVE" + this._engine.current_color());

          if (this._selectedX === -30) {
              return (new Move(this._selectedX, this._selectedY,null));

          } else {

          return (new Move(this._selectedX, this._selectedY, new Building(this._engine.current_color(), this._engine._selectedBuildingType, new Coordinates(this._selectedX, this._selectedY))));
  ...
  _on_click(x,y) {
      if (x===-1 && y ===-1) {
          ...
          //console.log("COLOR ON CLICK EVENT");
          //console.log(this._engine.current_color());

          //console.log(this._engine._board[this._engine.getIndex(x, y)]);

          if (this.color() === this._engine.current_color()) {
                      ...
                      if (!this._engine.moved) {
                          for (let i = 0; i < this._engine._architects.length; i++) {
                              ...
                              if (x == this._engine._architects[i]._coord.x() && y == this._engine._architects[i]._coord.y()) {
                                  console.log("ARCHITECTTTTTTT CLICKED");
                                  this._engine._phase = Phase.ARCHITECT;
                  ...
                  case Phase.BUILDING:
                      ...
                      for (let i = 0; i < interList.length; i++) {
                          if (interList[i]._coord.x() == x && interList[i]._coord.y() == y) {
                              ...
                              //this._color=this._engine.current_color();
                          }
