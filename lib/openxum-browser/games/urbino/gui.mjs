...
class Gui extends OpenXum.Gui {
    ...
    draw() {
        ...
        this._draw_piece(1,1, BuildingType.HOUSE);
        this._draw_piece(2,1, BuildingType.HOUSE);
        this._draw_piece(3,1, BuildingType.HOUSE);
        this._draw_piece(4,1, BuildingType.HOUSE);
        this._draw_piece(5,1, BuildingType.HOUSE);
        this._draw_piece(6,1, BuildingType.HOUSE);

        this._draw_piece(2,2, BuildingType.PALACE);

        this._draw_piece(3,3, BuildingType.TOWER);
        this._draw_piece(4,4, BuildingType.TOWER);
        this._draw_piece(5,5, BuildingType.TOWER);
        this._draw_piece(6,6, BuildingType.TOWER);
        this._draw_piece(7,7, BuildingType.TOWER);
        this._draw_piece(8,8, BuildingType.TOWER);

        this._draw_piece(9,9, BuildingType.TOWER);
        //console.log(BuildingType.HOUSE);
    ...
    _draw_piece(x,y,piece)
    {
       ...
       let radius = (this._deltaX / 5); // Créer un rond de taille
        ...
        if(piece == Architecte)
        {
            ...
            this._draw_architecte(x,y);

        }

        if(piece === BuildingType.HOUSE)
        {
            this._draw_house(x,y);
        }

        if(piece === BuildingType.PALACE)
        {
            this._draw_palace(x,y);
        }

        if(piece === BuildingType.TOWER)
        {
            this._draw_tower(x,y);
        }

        /*else
        {
            switch (piece._BuildingType()) {

                case HOUSE : this._draw_house(x,y); break;
                case PALACE : this._draw_palace(x,y); break;
                case TOWER : this._draw_tower(x,y);break;
            }
        }*/
